...
let AUTH_HASH = null
bcrypt.hash(AUTH_PASS, 12).then(h => {
  AUTH_HASH = h
  console.log('[abflow-backend] password hashed, ready')
})
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
  ...
  if (!AUTH_HASH) return res.status(503).json({ error: 'Server initializing, retry in a moment' })
